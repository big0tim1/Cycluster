...
def plotCorrHeatmap(df=None, metric='pearson', rowInd=None, colInd=None, col_labels=None, titleStr=None, vRange=None,
                    tickSz='large', cmap=None, dmat=None, cbLabel='Correlation', minN=1):
...
def plotHColCluster(df=None, col_dmat=None, method='complete', metric='euclidean', col_labels=None, titleStr=None,
                    vRange=None, tickSz='medium', cmap=None, minN=1, K=None, labelCmap=None, noColorBar=False,
                    interactive=False):
    ...
    if not col_labels is None:
        ...
        col_axi = col_cbAX.imshow([[x] for x in col_cbSE.iloc[colInd].values], interpolation='nearest', aspect='auto',
                                  origin='lower')
        clean_axis(col_cbAX)
...
def plot1DHClust(distDf, hclusters, labels=None, titleStr=None, vRange=None, tickSz='small', cmap=None,
                 colorbarLabel=None, labelCmap=None, noColorBar=False):
...
def plotHCluster(df, method='complete', metric='euclidean', clusterBool=[True, True], row_labels=None, col_labels=None,
                 vRange=None, titleStr=None, xTickSz='small', yTickSz='small', cmap=None, minN=1):
    ...
    fig.clf()
    if clusterBool[1]:
        heatmapGS = gridspec.GridSpec(3, 3, wspace=0.0, hspace=0.0, width_ratios=[0.15, 0.02, 1],
                                      height_ratios=[0.15, 0.02, 1])
    else:
        heatmapGS = gridspec.GridSpec(3, 3, wspace=0.0, hspace=0.0, width_ratios=[0.15, 0.02, 1],
                                      height_ratios=[0.001, 0.02, 1])
    ...
    if not row_labels is None:
        ...
        row_axi = row_cbAX.imshow([[x] for x in row_cbSE.iloc[rowInd].values], interpolation='nearest', aspect='auto',
                                  origin='lower')
        clean_axis(row_cbAX)
    ...
    axi = heatmapAX.imshow(df.iloc[rowInd, colInd], interpolation='nearest', aspect='auto', origin='lower',
