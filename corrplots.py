...
def partialcorr(x, y, adjust=[], method='pearson', minN=None):
    ...
    try:
        ...
        if len(adjust) > 0:
            warnings.warn(
                'Error computing %s and %s correlation: using scipy equivalent to return UNADJUSTED results' % (
                x.name, y.name))
        else:
    ...
    for r in range(n):
        for c in range(n):
            if r == c:
                ...
                plt.axis([rmin - 0.1 * (rmax - rmin), rmax + 0.1 * (rmax - rmin), cmin - 0.1 * (cmax - cmin),
                          cmax + 0.1 * (cmax - cmin)])
            elif r < c:
                axh[r, c] = fh.add_subplot(gs[r, c], yticklabels=[], xticklabels=[], xticks=[], yticks=[])
    ...
    for i, rowv in enumerate(rowVars):
        for j, colv in enumerate(colVars):
            if not rowv == colv:
                ...
                if not N[i, j] < minN:
                    rho[i, j], pvalue[i, j] = partialcorr(df[rowv], df[colv], adjust=[df[a] for a in adjust],
                                                          method=method)
                else:
...
def corrheatmap(df, rowVars=None, colVars=None, adjust=[], annotation=None, cutoff=None, cutoffValue=0.05,
                method='pearson', labelLookup={}, xtickRotate=True, labelSize='medium', minN=0, adjMethod='fdr_bh'):
    ...
    rho, pvalue, qvalue, N = pwpartialcorr(df, rowVars=rowVars, colVars=colVars, adjust=adjust, method=method,
                                           minN=minN)
    ...
    method = method[0].upper() + method[1:]
    plt.annotate('%s correlation' % method, [0.98, 0.5], xycoords='figure fraction', ha='right', va='center',
                 rotation='vertical')
    return rho, pvalue, qvalue


def scatterfit(x, y, method='pearson', adjustVars=[], labelLookup={}, plotLine=True, plotUnity=False, annotateFit=True,
               annotatePoints=False, returnModel=False, lc='gray', **kwargs):
